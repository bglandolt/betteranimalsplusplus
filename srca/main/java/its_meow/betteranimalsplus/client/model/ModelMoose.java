...
public class ModelMoose extends ModelBase {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entity)
    {
        ...
        this.lForeleg01.rotateAngleX = MathHelper.cos(limbSwing * 0.6662F) * 1.4F * f1 + 0.19198621771937624F;
        this.rForeleg01.rotateAngleX = MathHelper.cos(limbSwing * 0.6662F + (float) Math.PI) * 1.4F * f1 + 0.19198621771937624F;
        this.rHindLeg01.rotateAngleX = MathHelper.cos(limbSwing * 0.6662F) * 1.4F * f1 - 0.22759093446006054F;
        this.lHindLeg01.rotateAngleX = MathHelper.cos(limbSwing * 0.6662F + (float) Math.PI) * 1.4F * f1 - 0.22759093446006054F;
        ...
        // if ( entity instanceof EntityMoose )
        {
            ...
            float defaultNeck = ModelBetterAnimals.getHeadPitch((EntityLiving) entity) * 0.017453292F - 0.07F + MathHelper.sin(limbSwing * 0.3F) * 0.06F;
            // float sway = MathHelper.sin(limbSwing * 0.3F) * 0.06F + 0.03F;

            if ( moose.headRam > 0 ) // 20
            {
            	if ( moose.headRam >= 8 ) // Head starts down. 20 to 4, raise head up!
            	{
            		this.head.rotateAngleX = -(float)(20-moose.headRam) * 0.04F + defaultHead + 0.5F;
                    this.neck.rotateAngleX = -(float)(20-moose.headRam) * 0.04F + defaultNeck + 0.5F;
	                this.head.rotateAngleZ =  moose.headDirection * (float)(20-moose.headRam) * 0.025F;
            	}
            	else if ( moose.headRam >= 4 )// Head is up, and goes up!
            	{
            		this.head.rotateAngleX = -(float)(8-moose.headRam) * 0.04F + defaultHead;
                    this.neck.rotateAngleX = -(float)(8-moose.headRam) * 0.04F + defaultNeck;
                	this.head.rotateAngleZ =  moose.headDirection * (float)(moose.headRam-8) * 0.0375F + (0.3F * moose.headDirection);
            	}
            	else // Head is up, and goes down to normal...
            	{
            		this.head.rotateAngleX = -(float)(moose.headRam-4) * 0.04F + defaultHead - 0.16F;
                    this.neck.rotateAngleX = -(float)(moose.headRam-4) * 0.04F + defaultNeck - 0.16F;
                	this.head.rotateAngleZ =  moose.headDirection * (float)(moose.headRam-8) * 0.0375F + (0.3F * moose.headDirection);
            	}
            }
            else if ( moose.headDownDuration > 0 )
            {
            	if ( moose.headDownDuration >= 90 ) // Head start up. 120 to 90 head going down.
            	{
            		this.head.rotateAngleX = (float)(120-moose.headDownDuration) * 0.0167F + defaultHead;
            		this.neck.rotateAngleX = (float)(120-moose.headDownDuration) * 0.0167F + defaultNeck;
            	}
            	else if ( moose.headDownDuration <= 30 ) // Head start down. 30 to 0 head going up.
            	{
            		this.head.rotateAngleX = -(float)(30-moose.headDownDuration) * 0.0167F + defaultHead + 0.5F;
                    this.neck.rotateAngleX = -(float)(30-moose.headDownDuration) * 0.0167F + defaultNeck + 0.5F;
            	}
...
//            	**moose.headDownDuration--;
            ...
            else if ( eatTime > 0 ) // 60
            {
            	if ( eatTime > 40 )
            	{
            		...
            		this.neck.rotateAngleX = -(float)(eatTime-40) * 0.035F + defaultHead;
            	}
            	else if ( eatTime < 30 )
            	{
            		this.head.rotateAngleX = -(float)(30-eatTime) * 0.0213F + 1.165F;
                    this.neck.rotateAngleX = -(float)(30-eatTime) * 0.0233F + 0.7F + defaultNeck;
            	}
            	else
            	{
            ...
            this.chest.rotateAngleZ = MathHelper.cos(limbSwing * 0.35F) * 0.05F * f1;
