...
//        return r;
    ...
    public void onLivingUpdate()
    {
        ...
        {
            if ( getMyMovementSpeed(this) > 0.16F )
            {
                float velX = (float) (0.5F * Math.cos(((this.rotationYaw - 90.0F) % 360.0F) / 60.0F));
                float velZ = (float) (0.5F * Math.sin(((this.rotationYaw - 90.0F) % 360.0F) / 60.0F));
                this.motionX -= velX;
                this.motionZ -= velZ;
                this.motionY = 0.34D;
                this.readyToJumpTimer = this.rand.nextInt(6) + 6;
                this.onGround = false;
    ...
    public void getHostiles()
    {
        ...
        for ( EntityPlayer player : list )
        {
        	...
        	if ( this.getDistance(player) < (player.isSneaking()?6.0D:10.0D) + 8.0D * ( abs(player.motionX) + abs(player.motionY) + abs(player.motionZ) ) )
        	{
        		this.setAttackTarget(player);
            	return;
        	}
