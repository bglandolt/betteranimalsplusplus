        	...
        	// if ( dist <= 256.0D )
	    	{
	    		if ( this.attackTick == 16 )
                {
	    			if ( !this.leapLocationLockedIn() ) // Set jump location!
                	{
				        ...
				        if ( this.bigLeap )
				        {
				        	this.attacker.playSound(SoundEvents.ENTITY_BAT_AMBIENT, 0.9F+rand.nextFloat()/10.0F, 0.8F+rand.nextFloat()/5.0F);
				        }
	                ...
	                if ( this.attacker.canEntityBeSeen(entity) )
	                {
				        ...
				        if ( blocksAboveTarget > entity.height * 2.0D )
		                {
					        if ( dist < 9.0 )
				        	{
				        		...
				        		this.attacker.motionY += 0.2D * ystrength;
					        	return;
				        	}
				        	else
				        	{
		                ...
		                else if ( blocksAboveTarget > entity.height/2.0D )
		                {
				        	...
				        	{
				        		this.attacker.motionY -= 0.1D * ystrength;
				        	}
		                }
		                else
		                {
...
//	    	}
        ...
        private void resetAttack(double dist)
        {
        	...
        	if ( dist <= 20 && dist >= 9 && this.attacker.getAttackTarget() != null && this.attacker.posY - this.attacker.getAttackTarget().posY < this.attacker.getAttackTarget().height )
        	{
    			this.bigLeap = true;
