...
//    }
    ...
    {
        this.playSound(SoundEvents.ENTITY_POLAR_BEAR_AMBIENT, 1.0F, 0.9F);
        return null;
    }
    ...
    {
        this.playSound(SoundEvents.ENTITY_POLAR_BEAR_HURT, 0.7F, 0.8F);
        return null; 
    }
    ...
    {
        this.playSound(SoundEvents.ENTITY_POLAR_BEAR_DEATH, 0.7F, 0.8F);
        return null; 
    }
    ...
    static class MoveHelperController extends EntityMoveHelper {
        ...
        private void updateSpeed()
        {
            ...
            {
            	if ( this.walrus.ticksExisted % 20 == 0 && this.walrus.motionX*this.walrus.motionX+this.walrus.motionZ*this.walrus.motionZ >= 0.0001D )
            	{
            		try
            		{
            			PathPoint p = this.walrus.getNavigator().getPath().getFinalPathPoint();
            			
            			double x = p.x - this.walrus.posX;
            			double z = p.z - this.walrus.posZ;
            			
            			double ratio = (x*x+z*z);
            			...
            			this.walrus.addVelocity(x/ratio/4.0D, 0.28D, z/ratio/4.0D);
            			this.walrus.world.setEntityState(this.walrus, (byte) 27);
            	        this.walrus.playSound(SoundEvents.ENTITY_SLIME_JUMP, 0.5F, 0.8F);
            			//this.walrus.leaping = 16;
                ...
                this.walrus.setAIMoveSpeed(Math.max(this.walrus.getAIMoveSpeed() / 2.0F, 0.1F));
            }
        ...
        public void onUpdateMoveHelper() {
            ...
            if(this.action == EntityMoveHelper.Action.MOVE_TO && !this.walrus.getNavigator().noPath()) {
                ...
                this.walrus.setAIMoveSpeed(lerp(0.135F, this.walrus.getAIMoveSpeed(), f1));
