...
//    }
	...
	public static void spawnSweepHit( Entity e, Entity target )
    {
        ...
        if (e.world instanceof WorldServer)
        {
            ((WorldServer)e.world).spawnParticle(EnumParticleTypes.SWEEP_ATTACK, (target.posX+e.posX)/2.0D + d0, target.posY + (double)target.height * 0.5D, (target.posZ+e.posZ)/2.0D + d1, 0, d0, 0.0D, d1, 0.0D);
        }
    }
