...
public class CivilizationHandlers
{
	...
	protected static int SPAWN_HEIGHT = ToroQuestConfiguration.spawnHeight;
	...
	{
		if ( player == null || province == null || province.civilization == null || player.dimension != 0 )
		{
			return;
		}
...
//		}
	...
	public static String repLevelMessage(EntityPlayer player, CivilizationType civ, int startRep, int afterRep)
	{
		...
		else if ( after == ReputationLevel.HONORED )	// 100
		{
			...
			"§r. You may butcher livestock without losing reputation!";
		}
		...
		else if ( after == ReputationLevel.CHAMPION )	// 1000
		{
			...
			"§r. You have unlocked dangerous and rewarding Legendary Quests!";
		}
		else if ( after == ReputationLevel.HERO )		// 2000
		{
			s = "§rYou are now a §oHero§r of §lHouse " + CivilizationType.civServerName(civ.getCivName()) + 
					"§r. Quests rewards are doubled!";
		}
		else if ( after == ReputationLevel.LEGEND )		// 3000
		{
			...
			"§r. You have achieved the highest reputation rank, and quests will grant experience in addition to their rewards!";
...
//					}
	...
	public static boolean isBuilding(Block block)
	{
		return     
				...
				// || (block.getDefaultState() == Block.getBlockFromName("")); // TODO
	}
...
//		World world = player.getEntityWorld();
		...
		if ( players > 0 )
		{
			...
			if ( ToroQuestConfiguration.caravanSpawnRate > 0 && event.world.getWorldTime() <= 11000 && (rand.nextInt(100)) < ToroQuestConfiguration.caravanSpawnRate + MathHelper.clamp((players-1)*2, 0, ToroQuestConfiguration.caravanSpawnRate) )
	    	{
...
//					}
					...
					if ( !(world.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB(banditSpawnPos).grow(25, 25, 25))).isEmpty() )
					{
						continue;
					}
		...
		try
		{
			...
			while ( tries > 0 )
			{
				...
				for ( EntityPlayer player : players )
				{
					...
					Village village = world.getVillageCollection().getNearestVillage(player.getPosition(), 512);
		...
		try
		{
			...
			while ( tries > 0 )
			{
				...
				for ( EntityPlayer player : players )
				{
					...
					int villageCenterZ = (int)player.posZ;
...
//					}
					...
					if ( !(world.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB(banditSpawnPos).grow(32, 32, 32))).isEmpty() )
					{
						continue;
					}
