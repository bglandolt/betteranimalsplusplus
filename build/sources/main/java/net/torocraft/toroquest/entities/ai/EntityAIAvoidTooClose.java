...
public class EntityAIAvoidTooClose<T extends Entity> extends EntityAIBase
{
    ...
    {
    	if ( this.entity.getAttackTarget() != null || this.rand.nextInt(30) != 0 )
    	{
    		return false;
    	}
    	...
    	if ( rand.nextBoolean() )
        {
	    	List<EntityPlayer> players = this.entity.world.<EntityPlayer>getEntitiesWithinAABB(EntityPlayer.class, this.entity.getEntityBoundingBox().grow(4.5D, 3.0D, 4.5D));
	    	...
	    	for ( EntityPlayer player : players )
	        {
	    		...
	    		CivilizationType civ = this.entity.getCivilization();
	    		
	    		if ( civ != null )
	    		{
		    		int rep = PlayerCivilizationCapabilityImpl.get(player).getReputation(civ);
	
		    		if ( rep <= -50 || (this.entity.underAttack() == player) || (this.entity.murderWitness() == player) )
		    		{
		    			if ( this.entity.actionReady() )
		    			{
		    				if ( this.rand.nextBoolean() )
		    				{
		                		this.entity.insult(player);
		    				}
		    			}
		    			else
		    			{
			    			this.entity.setAnnoyed(player);
			    			this.entity.setAttackTarget(player);
		    			}
		    			return false;
		    		}
	    		}
	    		
            	if ( player.getDistanceSq(this.entity) >= 1.75D )
            	{
            		if ( this.entity.actionReady() )
                	{
	            		if ( this.rand.nextInt(4) == 0 )
	            		{
	            			EntityGuard.guardSpeak(this.entity, player);
	            		}
	            		else
	            		{
	            			this.entity.actionTimer = 4;
	            		}
            		}
            	}
            	else
