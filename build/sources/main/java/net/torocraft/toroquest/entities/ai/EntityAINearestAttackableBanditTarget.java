...
public class EntityAINearestAttackableBanditTarget extends EntityAITarget
{
			...
			public boolean apply(@Nullable EntityLiving target)
			{
				...
				if ( !taskOwner.playerGuard.equals("") )
				{
					...
					if ( target instanceof IMob || target instanceof EntityMob )
					{
						if ( target instanceof EntitySentry && !(target instanceof EntityOrc) )
						{
							...
							if ( t.getTame() || t.spawnedNearBandits )
							{
								return false;
							}
							else
				...
				else
				{
					if ( target instanceof IMob || target instanceof EntityMob )
					{
						...
						if ( taskOwner.getDistance(target) <= 8 || (taskOwner.raidX != null && taskOwner.raidZ != null && taskOwner.getDistance(taskOwner.raidX, taskOwner.posY, taskOwner.raidZ) <= 24) )
						{
							return true;
						}
					}
	...
	public boolean shouldExecute()
	{
		...
		else
		{
			...
			for ( EntityLiving npc : list )
			{
				if ( npc instanceof EntityToroMob || npc.getAttackTarget() instanceof EntityVillager || npc.getAttackTarget() instanceof EntityToroNpc || this.taskOwner.getDistance(npc) <= 6.0D )
				{
					this.targetEntity = npc;
					return true;
				}
