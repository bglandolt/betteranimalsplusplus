...
public class EntitySentry extends EntityToroMob implements IRangedAttackMob, IMob
{
    ...
    private static final AttributeModifier MODIFIER = (new AttributeModifier(MODIFIER_UUID, "Drinking speed penalty", -0.35D, 0)).setSaved(false);
    private static final DataParameter<Boolean> IS_DRINKING = EntityDataManager.<Boolean>createKey(EntitySentry.class, DataSerializers.BOOLEAN);
	...
	public void chat( EntityPlayer player, String message, @Nullable String extra )
	{
		if ( ToroQuestConfiguration.guardsHaveDialogue )
		{
			...
			try
			{
				String s = "§l" + this.getName() + "§r: " + (I18n.format("entity.toroquest.sentry."+message+rand.nextInt(Integer.parseInt(I18n.format("entity.toroquest.sentry."+message)))).replace("@p", player.getName()));
	...
	{
		if ( player == null || hand == null || !this.isEntityAlive() )
    	{
			return false;
    	}
    		...
    		List<EntitySentry> bandits = this.world.<EntitySentry>getEntitiesWithinAABB(EntitySentry.class, new AxisAlignedBB(this.getPosition()).grow(32, 16, 32));
    		{
    			for ( EntitySentry bandit : bandits )
    			{
    				if ( bandit.passiveTimer <= 0 )
    				{
        				...
        				bandit.getNavigator().tryMoveToEntityLiving(player, 0.45D);
    				}
			...
			if ( this.getTame() )
			{
	        	if ( !world.isRemote )
	        	{
					...
					if ( province != null && province.getCiv() != null )
					{
						...
						newEntity.chat(newEntity, player, "civbanditrecruit", province.getCiv().getDisplayName());
					}
					else
					{
	...
	protected void initEntityAI()
	{
		...
		this.tasks.addTask(3, new EntityAIFlee(this, 0.7D));
        ...
        this.targetTasks.addTask(1, new EntityAIHurtByTarget(this, true, new Class[0])
		{
			...
			public boolean shouldExecute()
			{
		        ...
		        return attacker != null && this.isSuitableTarget(attacker, false) && !(attacker instanceof EntitySentry);
			}
		});
	}
	...
	public void onLivingUpdate()
	{
		...
		if ( this.isEntityAlive() && this.getHealth() > 0 )
		{
			if ( this.isDrinkingPotion() )
	        {
	            ...
	            if ( this.potionUseTimer >= 12 && this.potionUseTimer % 4 == 0 )
	            {
	            	if ( this.getAttackTarget() != null )
	            	{
						...
						if ( !world.isRemote && !this.isAirBorne && dist <= 20 )
						{
			            	float push = (float)((8+dist)*256);
	            	...
	            	this.world.playSound((EntityPlayer)null, this.posX, this.posY, this.posZ, SoundEvents.ENTITY_GENERIC_DRINK, this.getSoundCategory(), 0.4F, 0.8F + this.rand.nextFloat() * 0.4F);
	            	if ( rand.nextInt(3) == 0 )
	            	{
	        ...
	        else if ( this.limitPotions > 0 && this.useHealingPotion && this.getHealth()/2.0F <= this.getMaxHealth() & this.getAttackTarget() != null && this.getDistance(this.getAttackTarget()) >= 4 )
	        {
	        	this.getNavigator().clearPath();
	        	this.getLookHelper().setLookPositionWithEntity(this.getAttackTarget(), 30.0F, 30.0F);
	        	this.faceEntity(this.getAttackTarget(), 30.0F, 0.0F);
	...
	public boolean attackEntityFrom(DamageSource source, float amount)
	{
		...
		if ( e == null )
		{
			if ( !(this instanceof EntityOrc) && this.rand.nextBoolean() )
			{
...
//		setEnchantmentBasedOnDifficulty(difficulty);
	...
	public void setMount()
	{
		...
		if ( this instanceof EntityOrc )
		{
			chance = ToroQuestConfiguration.orcMountChance;
		}
	...
	private void sentryTypeDPS( )
	{
		if ( this.getAttackTarget() != null && this.getAttackTarget().isEntityAlive() && !(this.getAttackTarget() instanceof EntityToroMob) )
		{
			...
			if ( this.limitPotions > 0 && !this.isDrinkingPotion() )
			{
				...
				{
					if ( world.getEntitiesWithinAABB(EntityToroMob.class, new AxisAlignedBB(this.getPosition()).grow(3, 3, 3)).size() < 2 )
					{
				...
				{	
					if ( dist >= 10 && dist <= 80 )
					{
						if ( this.canEntityBeSeen(this.getAttackTarget()) && world.getEntitiesWithinAABB(EntityToroMob.class, new AxisAlignedBB(this.getPosition()).grow(3, 3, 3)).size() < 2 )
						{
