...
public class EntityCaravan extends EntityToroVillager implements IMerchant
{
	...
	public void addCaravan()
	{
		if ( !this.world.isRemote )
		{
			...
			if ( rand.nextInt(4) == 0 )
			{
				for ( int i = rand.nextInt(4)+4; i > 0; i-- ) mule.replaceItemInInventory(500+rand.nextInt(18), new ItemStack((Items.BREAD),rand.nextInt(5)+1));
				for ( int i = rand.nextInt(4)+4; i > 0; i-- ) mule.replaceItemInInventory(500+rand.nextInt(18), new ItemStack((Items.WHEAT),rand.nextInt(5)+1));
			}
			else if ( rand.nextInt(4) == 0 )
			{
				for ( int i = rand.nextInt(4)+4; i > 0; i-- ) mule.replaceItemInInventory(500+rand.nextInt(18), new ItemStack((Items.EMERALD),rand.nextInt(5)+1));
			}
			else if ( rand.nextInt(4) == 0 )
			{
				for ( int i = rand.nextInt(3)+3; i > 0; i-- ) mule.replaceItemInInventory(500+rand.nextInt(18), new ItemStack((Blocks.WOOL),rand.nextInt(5)+1));
				for ( int i = rand.nextInt(4)+4; i > 0; i-- ) mule.replaceItemInInventory(500+rand.nextInt(18), new ItemStack((Items.LEATHER),rand.nextInt(5)+1));
			}
			else
			{
				for ( int i = rand.nextInt(4)+4; i > 0; i-- ) mule.replaceItemInInventory(500+rand.nextInt(18), new ItemStack((Items.APPLE),rand.nextInt(5)+1));
			}
