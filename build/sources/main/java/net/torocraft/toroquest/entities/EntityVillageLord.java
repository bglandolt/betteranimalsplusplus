	...
	protected boolean processInteract(EntityPlayer player, EnumHand hand) 
	{
		...
		if ( this.isEntityAlive() && !this.isChild() )
		{
			...
			if ( rep > -50 && (this.murderWitness == null || this.murderWitness != player) && (this.underAttack == null || this.underAttack != player ) )
			{
				if ( this.isAnnoyed() )
				{
					if ( this.actionReady() )
					{
						...
						if ( !player.world.isRemote ) this.chatfriendly("annoyed", player, null);
					}
				}
				else if ( this.recentlyHit < 1 )
				{
			        ...
			        if ( itemstack.getItem() == Item.getByNameOrId("toroquest:city_key") && !(name.equals("Key to the City")) )
			        {
			        	...
			        	{	
							...
							if ( !player.world.isRemote ) this.chatfriendly("rename", player, name);
			        	}
			        	else
			        	{
			        		if ( !player.world.isRemote ) this.chatfriendly("renameconsider", player, name);
			        	}
			        }
			        else
			        {
			...
			else
			{
				...
				if ( !this.isAnnoyed() )
				{
					if ( !player.world.isRemote ) this.chat(this, player, "crime", null);
				}
