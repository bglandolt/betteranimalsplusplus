	...
	public void onEntitySpawn(EntityJoinWorldEvent event)
	{
		...
		if ( entity instanceof EntityMob )
		{
			 ...
			 */
				...
				if ( ( mob.getPosition().getY() >= 40 && Math.abs(villageCenterX-entityPosX) <= allowedDistance && Math.abs(villageCenterZ-entityPosZ) <= allowedDistance ) )
				{
					mob.setDead();
					event.setCanceled(true);
					return;
...
//			}
			...
			if ( pos != null )
			{
				if ( ((EntityPotion)event.getEntity()).getThrower() instanceof EntitySentry )
				{
...
//		System.out.println("hit: " + entity);
		...
		else if ( entity instanceof EntityToroMob )
		{
			...
			{
				EntityArrow arrow = (EntityArrow)event.getEntity();
				if ( arrow.shootingEntity instanceof EntityToroMob )
				{
					event.setCanceled(true);
					return;
				}
	...
	public void livingHurtEvent(LivingHurtEvent event)
	{
		...
		if ( attacker instanceof EntityGuard )
		{
			...
			{
				event.setAmount( event.getAmount() * (ToroQuestConfiguration.guardDamageMultiplierToPlayers+(rand.nextFloat()-0.5F)/5.0F) );
			...
			{
				event.setAmount( event.getAmount() * (ToroQuestConfiguration.guardDamageMultiplierToMobs+(rand.nextFloat()-0.5F)/5.0F) );
				Province province = CivilizationUtil.getProvinceAt( attacker.getEntityWorld(), attacker.chunkCoordX, attacker.chunkCoordZ );
				if ( province == null )
				{
					event.setAmount(event.getAmount()*ToroQuestConfiguration.guardNerfOutsideProvince);
		...
		else if ( attacker instanceof EntityToroMob )
		{
			...
			{
				event.setAmount( event.getAmount() * (ToroQuestConfiguration.orcDamageMultiplier+((rand.nextFloat()-0.5F)/5.0F)) );
			}
			else // if ( !(attacker instanceof EntityBanditLord) )
			{
				event.setAmount( event.getAmount() * (ToroQuestConfiguration.banditDamageMultiplier+ToroQuestConfiguration.banditDamageMultiplier+((rand.nextFloat()-0.5F)/5.0F)) );
			}
		}
...
//			}
	...
	protected void teleportRandomly(EntityPlayer p)
    {
		...
		p.attemptTeleport(pos.getX(), pos.getY(), pos.getZ());
    }
