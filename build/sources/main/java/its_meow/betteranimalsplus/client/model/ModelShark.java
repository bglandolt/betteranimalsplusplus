...
public class ModelShark extends ModelBase {
    ...
    public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
    {
        if(entity instanceof EntityShark)
        {
            ...
            if ( Math.abs(entity.motionY) > 0.01 && ( Math.abs(entity.motionX) > 0.01 || Math.abs(entity.motionZ) > 0.01 ) )
            {
                //float rotX = -((float) Math.atan(entity.motionY / Math.sqrt(Math.pow(entity.motionX, 2) + Math.pow(entity.motionZ, 2))) / 1.5F);
                float rotX = -((float) Math.atan2(shark.motionY, Math.sqrt(Math.pow(shark.motionX, 2) + Math.pow(shark.motionZ, 2))) / 2.0F);
